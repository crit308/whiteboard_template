...
export async function POST(req: NextRequest) {
  ...
  const received = auth?.slice(0, 20);
  const expected = `Bearer ${process.env.OVERLAY_SECRET}`.slice(0, 20);
  console.log(`[overlay] Auth header starts with: ${received}, expected starts with: ${expected}`);
  if (!process.env.OVERLAY_SECRET) {
    console.warn('[overlay] OVERLAY_SECRET not set in env – skipping auth');
  } else if (auth !== `Bearer ${process.env.OVERLAY_SECRET}`) {
